...
(function($) {
		if ($.jQTouch) {
				$.jQTouch.addExtension(function Calendar(jQT){
					...
					Zepto.fn.getCalendar = function(options) {
						var defaults = {
										date	: new Date(),
...
//Markup Reading
 ...
 //Markup Generation
					Zepto.fn.dayMarkup = function(format,day,month,year,column) {
						var this_day = $('<td/>');
					...
					Zepto.fn.monthLength = function(month, year) {
						var dd = new Date(year, month, 0);
						return dd.getDate();
					}
					Zepto.fn.monthMarkup= function(month, year) {
						var $el = $(this);
					...
					Zepto.fn.generateCalendar = function(month, year) {
						var $el = $(this);
...
//Tasks/Events
					Zepto.fn.attachEvents = function() {
						return this.each(function(){
							var $el = $(this);
					...
					Zepto.fn.hasEvent = function(date) {
					...
					Zepto.fn.getCellDate = function(dateCell) {
						var date = $(dateCell).attr('datetime');
						return $(this).stringToDate(date);
					}
					Zepto.fn.stringToDate = function(dateString) {
						var a = dateString.split('-');
						return new Date(a[0],(a[1]-1),a[2]);
					}
					Zepto.fn.dateToString = function(date) {
						return date.getFullYear()+"-"+(date.getMonth() + 1)+"-"+date.getDate();
					}
					Zepto.fn.getEvents = function(date) {
						var $el = $(this);
						var d = date.getDate();
...
//DOM events & Calendar Manipulation
            ...
            if (Modernizr.touch) {
              clicktouch = "touchstart";
					...
					Zepto.fn.removeSelectedCell = function() {
							$(this).find('.selected').removeClass('selected');
					}

					Zepto.fn.setToday = function() {
						var $el = $(this);
//						var date = $el.data('settings').date;
					...
					Zepto.fn.setSelected = function(date) {
						$el = $(this);
						$el.removeSelectedCell();
					...
					Zepto.fn.sameDay = function(date1, date2) {
					...
					Zepto.fn.loadMonthDelta = function(delta) {
						$el = $(this);
